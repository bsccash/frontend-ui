...
/**
 * An API module of Bsc Cash contracts.
 * All contract-interacting domain logic should be defined in here.
 */
export class BscCash {
  myAccount: string;
  ...
  constructor(cfg: Configuration) {
    ...
    this.BCS = new ERC20(deployments.Cash.address, provider, 'BCS');
    this.BSS = new ERC20(deployments.Share.address, provider, 'BSS');
    this.BSB = new ERC20(deployments.Bond.address, provider, 'BSB');
  ...
  unlockWallet(provider: any, account: string) {
    ...
    const tokens = [this.BCS, this.BSS, this.BSB, ...Object.values(this.externalTokens)];
  ...
  /**
   * @returns Bsc Cash (BCS) stats from Uniswap.
   * It may differ from the BCS price used on Treasury (which is calculated in TWAP)
   */
  async getCashStatFromUniswap(): Promise<TokenStat> {
    const supply = await this.BCS.displayedTotalSupply();
    return {
      priceInDAI: await this.getTokenPriceFromUniswap(this.BCS),
      totalSupply: supply,
    };
  }

  /**
   * @returns Estimated Bsc Cash (BCS) price data,
  ...
  async getBondStat(): Promise<TokenStat> {
    ...
    return {
      ...
      totalSupply: await this.BSB.displayedTotalSupply(),
    };
  }

  async getShareStat(): Promise<TokenStat> {
    return {
      priceInDAI: await this.getTokenPriceFromUniswap(this.BSS),
      totalSupply: await this.BSS.displayedTotalSupply(),
    };
  }
  ...
  async fetchBoardroomVersionOfUser(): Promise<string> {
    ...
    if (balance1.gt(0)) {
      console.log(
        `👀 The user is using Boardroom v1. (Staked ${getDisplayBalance(balance1)} BSS)`,
      );
      return 'v1';
    }
    ...
    if (balance2.gt(0)) {
      console.log(
        `👀 The user is using Boardroom v2. (Staked ${getDisplayBalance(balance2)} BSS)`,
      );
      return 'v2';
    }
    return 'latest';
  ...
  async stakeShareToBoardroom(amount: string): Promise<TransactionResponse> {
    if (this.isOldBoardroomMember()) {
      throw new Error("you're using old Boardroom. please withdraw and deposit the BSS again.");
    }
